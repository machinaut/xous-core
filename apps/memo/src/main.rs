...
pub(crate) const SERVER_NAME_MEMO: &str = "_Voice Memo_";

/// Top level application events.
#[derive(Debug, num_derive::FromPrimitive, num_derive::ToPrimitive)]
pub(crate) enum MemoOp {
    /// Redraw the screen
    Redraw = 0,
...
impl Memo {
    fn new(xns: &xous_names::XousNames, sid: xous::SID) -> Self {
        ...
        let gam_token = gam
            .register_ux(gam::UxRegistration {
                app_name: xous_ipc::String::<128>::from_str(gam::APP_NAME_MEMO),
                ux_type: gam::UxType::Chat,
                predictor: None,
                listener: sid.to_array(),
                redraw_id: MemoOp::Redraw.to_u32().unwrap(),
                gotinput_id: None,
                audioframe_id: None,
                rawkeys_id: None,
                focuschange_id: None,
    ...
    fn redraw(&mut self) {
        ...
        write!(text_view.text, "{}", t!("memo.memo", xous::LANG)).expect("Could not write to text view");
        #[cfg(feature="tts")]
        self.tts.tts_simple(t!("memo.memo", xous::LANG)).unwrap();
...
fn main() -> ! {
    ...
    log::info!("Voice Memo PID is {}", xous::process::id());

    let xns = xous_names::XousNames::new().unwrap();

    // Register the server with xous
    let sid = xns
        .register_name(SERVER_NAME_MEMO, None)
        .expect("can't register server");

    let mut memo = Memo::new(&xns, sid);
